...
- `data/jobs.js` — 从 jobs.json 生成的 JS 包装（`file://` 协议兼容用）
...
├── data/
│   ├── jobs.json             # 招聘数据（JSON，源文件）
│   └── jobs.js               # 从 jobs.json 生成（file:// 兼容）
...
- **数据更新方式（桌面版）**：软件从 exe 旁边的 `data/jobs.json` 读取数据；首次运行会自动在 exe 旁生成一份 `data/`。**两种方式更新招聘信息**：①应用内编辑——顶栏「+ 新增」按钮、详情抽屉内「编辑/删除」按钮，保存后自动写入 exe 旁 `data/`（同时写 `jobs.json` 与 `jobs.js`，写前自动备份到 `data/backups/` 保留 10 份）；②直接改 exe 旁的 `data/jobs.json`（需同步重新生成 `jobs.js`）。两种方式都无需重新打包。
- **个人投递进度（仅桌面版）**：`data/my-status.json` 按 job id 存储个人状态（待投递/已投递/笔试/面试/offer/已拒绝）与备注，与共享数据分离。**分发给同学时不要带上此文件**。
- **应用内编辑的实现边界**：编辑入口仅在 `window.desktopAPI`（preload.js 提供）存在且数据目录可写时显示；网页版/file:// 打开时所有编辑入口隐藏。IPC 通道：`jobs:save`、`status:save`、`desktop:info`（见 main.js）。编辑器为独立文件 `js/editor.js`，通过 `window.JobApp`（app.js 暴露）读写数据。
...
- **排序** — 综合排序 / 最新发布（按 post_date）
- **卡片快捷投递** — 卡片直接显示内推码复制按钮与投递链接
- **内推码一键复制** — 点击复制，显示"已复制"反馈
- **详情抽屉** — 右侧滑入（移动端全屏），含薪资/学历/经验、二维码放大
- **应用内编辑（桌面版）** — 顶栏「+ 新增」、详情抽屉「编辑/删除」，保存即写盘生效，自动备份
- **个人投递进度（桌面版）** — 每条信息可标记状态与备注，卡片显示状态徽章，支持按进度筛选
